...
//  Project 23
//
//  Created by Артём Бурмистров on 5/22/20.
...
class GameScene: SKScene {
    ...
    func redrawActiveSlice() {
        ...
        for point in activeSlicePoints.dropFirst() {
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if randomPosition.x < 256 {
            randomXVelocity = .random(in: 8...15)
        } else if randomPosition.x < 512 {
            randomXVelocity = .random(in: 3...5)
    ...
    func tossEnemies() {
        ...
        case .chain:
            for i in 0..<5 {
                DispatchQueue.main
                    .asyncAfter(deadline: .now() + (chainDelay / 5.0 * Double(i)))
                    { [weak self] in
                        self?.createEnemy()
                    }
            }
        ...
        case .fastChain:
            for i in 0..<5 {
                DispatchQueue.main
                    .asyncAfter(deadline: .now() + (chainDelay / 10.0 * Double(i)))
                    { [weak self] in
                        self?.createEnemy()
                    }
            }
